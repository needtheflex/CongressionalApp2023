...
public partial class MainWindow : Window
{
    ...
    private void calculateSimpleInterest(object sender, RoutedEventArgs e)
    {
        ...
        {
            simpleInterestFutureValueUserInput.Text = (principle * (1 + (simpleRate / 100) * years)).ToString("0.00");
            return;
        }

        if (simpleRate == -69420)
        {
            simpleInterestRateUserInput.Text = ((((futureValue / principle) - 1) / years) * 100).ToString("0.00");
            return;
        }

        if (years == -69420)
        {
            simpleInterestYearsFutureUserInput.Text = (((futureValue / principle) - 1) / (simpleRate/100)).ToString("0.00");
            return;
        }
        return;
    }
    ...
    private void calculateCompoundInterest(object sender, RoutedEventArgs e)
    {
        ...
        if (emptyCount > 1)
        {
            compoundInterestValue.Text = "too many missing variables";
            return;
        }
        else if (emptyCount == 0)
        {
            compoundInterestValue.Text = "no variables to solve for!";
            return;
        }

        if (principle == -69420)
        {
            compoundInterestPrinciplePriceUserInput.Text = (futureValue / Math.Pow((1 + simpleRate), years)).ToString("0.00");
            return;
        }

        if (futureValue == -69420)
        {
            compoundInterestFutureValueUserInput.Text = (principle * Math.Pow(1 + simpleRate / 100, years)).ToString("0.00");
            return;
        }

        if (simpleRate == -69420)
        {
            compoundInterestRateUserInput.Text = (Math.Pow(futureValue / principle, 1 / years) - 1).ToString("0.00");
            return;
        }

        if (years == -69420)
        {
            compoundInterestYearsIntoFutureUserInput.Text = (Math.Log(futureValue / principle, 1 + simpleRate / 100)).ToString("0.00");
            return;
        }
        return;
